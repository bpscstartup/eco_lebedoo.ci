...
function adifier_admin_notices() {
	$adifier_builder_change = get_option( 'adifier_builder_change' );
	if( class_exists( 'KingComposer' ) && empty( $adifier_builder_change ) )
	{
		?>
		    <div class="notice notice-success is-dismissible error adifier-notice-dismiss" data-option="adifier_builder_change">
		        <p><?php esc_html_e( 'From version 3.9.1 default page builder becomes Elementor.', 'adifier' ); ?></p>
				<p><?php esc_html_e( 'KingComposer is also supported but we are strongly encourage you to move to Elementor.', 'adifier' ); ?></p>
				<p><?php esc_html_e( 'Once you install & activate, Elementor Appearance -> Import Demo Content will allow you to import demo content for Elementor', 'adifier' ); ?></p>
		    </div>
		<?php		
	}
	...
	if( function_exists('cmb_init') ):
	    ...
	    if( !empty( $smeta_data['smeta.php'] ) && $smeta_data['smeta.php']['Version'] != '1.6' ):
		    ?>
		    <div class="notice notice-success is-dismissible error">
	...
	if( function_exists('adifier_create_post_types') ):
	    ...
	    if( !empty( $smeta_data['adifier-system.php'] ) && $smeta_data['adifier-system.php']['Version'] != '3.1.3' ):
		    ?>
		    <div class="notice notice-success is-dismissible error">
	...
	if( function_exists('adifier_create_post_types') ):
	    ...
	    if( !empty( $smeta_data['adifier-import.php'] ) && $smeta_data['adifier-import.php']['Version'] != '1.4' ):
		    ?>
		    <div class="notice notice-success is-dismissible error">
	...
	if( !empty( adifier_get_option( 'gdpr_text' ) ) ):
        if( !class_exists( 'OMGF' ) ):
            ?>
            <div class="notice notice-success error">
                <p><?php esc_html_e( 'Theme recommends to install OMGF plugin to load fonts in GDPR compilant way. Plugins -> Add New, search form OMGF, install & activate', 'adifier' ); ?></p>
            </div>
            <?php
        endif;
        if( !class_exists( 'COMPLIANZ' ) && !function_exists('cky_set_default_language') && !class_exists( 'Moove_GDPR_Content' ) && !class_exists('Cookie_Notice') && !function_exists('BorlabsCookieHelper') ):
            ?>
            <div class="notice notice-success error">
                <p><?php esc_html_e( 'Theme supports following plugins to load 3rd party scripts (such as maps, search by location, etc...) in GDPR compliant way: Complianz | CookieYes | GDPR Cookie Compliance | Cookie Notice & Compliance for GDPR / CCPA | Borlabs Cookie. Go to Plugins -> Add New to install plugin and also read Adifier documentation section GDPR', 'adifier' ); ?></p>
            </div>
            <?php
        endif;
	endif;
	...
	if( empty( $update ) && class_exists( 'Redux' ) ){
		...
		foreach( $list as $item ){
			...
			Redux::set_option( 'adifier_options', $item, $data );
		}
...
function adifier_requred_plugins(){
	$plugins = array(
		...
		array(
				'name'                 => esc_html__( 'Elementor', 'adifier' ),
				'slug'                 => 'elementor',
				'required'             => false,
				'version'              => '',
				'force_activation'     => false,
...
if( !function_exists('adifier_ask_consent') ){
function adifier_ask_consent()
{
    if( empty( adifier_get_option( 'gdpr_text' ) ) )
    {
        return false;
    }
    else if( class_exists( 'COMPLIANZ' ) && ( empty( $_COOKIE['cmplz_functional'] ) || $_COOKIE['cmplz_functional'] != 'allow' ) )
    {
        return true;
    }
    else if( function_exists( 'cky_set_default_language' ) && ( empty( $_COOKIE['cookieyes-consent'] ) || !str_contains($_COOKIE['cookieyes-consent'], 'functional:yes') ) )
    {
        return true;
    }
    else if( class_exists( 'Moove_GDPR_Content' ) && !gdpr_cookie_is_accepted( 'thirdparty' ) )
    {
        return true;
    }
    else if( class_exists('Cookie_Notice') && ( empty( $_COOKIE['cookie_notice_accepted'] ) || $_COOKIE['cookie_notice_accepted'] != 'true' ) )
    {
        return true;
    }
    else if( function_exists('BorlabsCookieHelper') && BorlabsCookieHelper()->gaveConsent('borlabs-cookie') )
    {
        return true;
    }
    {
...
function adifier_map_script( $enqueue = false ){
    ...
    if( $map_source == 'google' && !$ask_map_consent ){
    ...
    else if( $map_source == 'mapbox' && !$ask_map_consent ){
        wp_enqueue_script( 'adifier-map', '//api.mapbox.com/mapbox-gl-js/v2.13.0/mapbox-gl.js', array('jquery'), false, true );
        ...
        wp_enqueue_style( 'mapbox', '//api.mapbox.com/mapbox-gl-js/v2.13.0/mapbox-gl.css' );
    ...
    else if( $map_source == 'osm' && !$ask_map_consent ){
    ...
    wp_localize_script( 'adifier-map-consent', 'adifier_map_data', array(
        'map_source' 			=> adifier_get_option( 'map_source' ),
...
function adifier_scripts_styles(){
	...
	if( ( is_page() && in_array( get_page_template_slug(), array( 'page-tpl_search.php', 'page-tpl_search_map.php' ) ) || is_tax( 'advert-category' ) || is_tax( 'advert-location' ) ) ){
		...
		$use_google_location = adifier_get_option( 'use_google_location' );
		if( $use_google_location == 'yes' && !adifier_ask_consent() ){
