...
class Adifier_Advert_Import{
		...
		if ( $this->addon->can_update_meta( 'advert_type', $import_options ) ) {
			if( !empty( $data['advert_type'] ) ){
				...
				if( !empty( $data['advert_videos'] ) ){
					...
					$delimiter = !empty( $data['advert_videos_delimiter'] ) ? $data['advert_videos_delimiter'] : '|';
					$videos = explode( $delimiter, $data['advert_videos'] );
					if( !empty( $videos ) ){
	...
	public function _add_fields(){
		...
		$this->addon->add_field( 'advert_videos', __( 'Videos', 'adifier' ), 'text', null, __( 'For multiple values in multiple tags divide with pipe "|" otherwise use delimiter', 'adifier' ) );
		$this->addon->add_field( 'advert_videos_delimiter', __( 'Videos Delimiter', 'adifier' ), 'text', null, __( 'If your file contains single videos tag input delimiter char.', 'adifier' ) );
