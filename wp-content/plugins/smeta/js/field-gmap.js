...
(function($) {
	...
	var CMBGmapsInit = function( fieldEl ) {
		...
		if( CMBGmaps.map_source == 'google' ){
			...
			if( CMBGmaps.country_restriction ){
				autocomplete.setComponentRestrictions({'country': CMBGmaps.country_restriction.split(',')});
			}
			...
			var geocoder = new MapboxGeocoder({
				...
				mapboxgl: mapboxgl,
				marker: false,
                language: CMBGmaps.geocoder_language,
                countries: CMBGmaps.country_restriction,
			});
			...
			geocoder.on('result', function( response ){
				if( response.result.place_type ){
					...
					setPosition( {lng: response.result.geometry.coordinates[0], lat: response.result.geometry.coordinates[1] }, 17 );
				}
			});
			...
			setTimeout(function(){
				...
				var onMap = false;
				window.results = [];
				var country_restriction = CMBGmaps.country_restriction;
				...
				$('.map-search').devbridgeAutocomplete({
					...
					params: {
                        ...
                        "accept-language": CMBGmaps.map_language
					},
					deferRequestBy: 1000
				});					
			}, 100);
