...
function adifier_send_subscription( $email = '' ){
	...
	if( !adifier_gdpr_given_consent() ){
        $response = [
            'status'   => 'error',
            'message'  => esc_html__( 'You have not gave consent', 'adifier' )
        ];
	}	
		...
		if( class_exists('Adifier_MailChimp') ){
			...
			if( !empty( $chimp_api ) && !empty( $chimp_list_id ) ){
				...
				if( $result === false) {
                    $response = [
                        'status'   => 'error',
                        'message'  => esc_html__( 'Failed to subscribe, try later', 'adifier' )
                    ];
				}
                    ...
                    $response = [
                        'status'   => 'success',
                        'message'  => esc_html__( 'You have been subscribed', 'adifier' )
                    ];
				}
                ...
                $response = [
                    'status'   => 'error',
                    'message'  => esc_html__( 'API not set', 'adifier' )
                ];
			}
		}
		else{
            $response = [
                'status'   => 'error',
                'message'  => esc_html__( 'No subscription provider', 'adifier' )
            ];
		}
	}
	else{
        $response = [
            'status'   => 'error',
            'message'  => esc_html__( 'Email is not valid', 'adifier' )
        ];
	}
...
function adifier_custom_meta(){
    ...
    $lang = $map_source == 'google' ? adifier_get_option( 'google_map_lang' ) : ( $map_source == 'mapbox' ? adifier_get_option( 'mapbox_map_lang' ) : adifier_get_option( 'osm_map_lang' ) );
	...
	$advert_contact = array(
		array(
            ...
            'map_language'			=> $lang,
            'geocoder_language'     => adifier_get_option( 'mapbox_geocoder_lang' ),
...
Add adifier foioter parts
*/
if( !function_exists('adifier_modals') ){
function adifier_modals(){
    include( get_theme_file_path( 'adifier-modals.php' ) );
}
add_action( 'wp_footer', 'adifier_modals' );
}
