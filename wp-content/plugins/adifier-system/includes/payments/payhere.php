...
class Adifier_PayHere{
	...
	*/
	static public function render(){
		?>	
		<li>
			<a href="javascript:void(0);" id="payhere-button" data-returnmessage="<?php esc_attr_e( 'Your payment is being processed and once it is approved you will receive purchased goods', 'adifier' ) ?>">
		...
		if( !empty( $_POST['data'] ) ){
			...
			foreach( $_POST['data'] as $key => $value ){
				if( empty( $value ) ){
					...
					$user_data[sanitize_text_field( $key )] = sanitize_text_field( $value );
				}
			}
    ...
    static public function verify_payment(){
    	...
    	if ( $local_md5sig === $_POST['md5sig'] && $_POST['status_code'] == 2 ){
			Adifier_Order::create_order(array(
				...
				'order_transaction_id' 	=> sanitize_text_field( $_POST['payment_id'] ),
