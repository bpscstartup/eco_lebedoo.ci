...
class Adifier_Order{
	...
	*/
	...
	static public function get_buyer_information( $order_id, $user_id )
	{
        ...
        $should_update = false;
        if( empty( $order_details['buyer_name'] ) )
        {
            $order_details['buyer_name'] = adifier_author_name( $user_id );
            $should_update = false;
        }
        if( empty( $order_details['buyer_tax_id'] ) )
        {
            $order_details['buyer_tax_id'] = get_user_meta( $user_id, 'tax_id', true );
            $should_update = true;
        } 
        ...
        if( adifier_get_option( 'enable_tax_id' ) == 'yes' )
        {
            $info[] = !empty($order_details['buyer_tax_id']) ? esc_html__( 'Tax ID: ', 'adifier' ) . $order_details['buyer_tax_id'] : '';
        }        
	...
	*/
	static public function order_save( $post_id, $post, $update ){
		...
		if( !isset( $_POST['adifier_refund_order'] ) ){
		    ...
		    else if( ( !empty( $old_paid ) && $old_paid == 'no' ) && $order_paid == 'canceled' ){
                $order_details = self::get_order_details( $post_id );
