...
$category 			= !empty( $_REQUEST['category'] ) 			? sanitize_text_field( $_REQUEST['category'] ) 				: '';
$location_id 		= !empty( $_REQUEST['location_id'] ) 		? sanitize_text_field( $_REQUEST['location_id'] ) 			: '';
$cf_fields			= !empty( $_REQUEST['cf_fields'] ) 			? map_deep( $_REQUEST['cf_fields'], 'sanitize_text_field' )	: array();
$latitude 			= !empty( $_REQUEST['latitude'] ) 			? sanitize_text_field( $_REQUEST['latitude'] ) 				: '';
$longitude 			= !empty( $_REQUEST['longitude'] ) 			? sanitize_text_field( $_REQUEST['longitude'] ) 			: '';
$location 			= !empty( $_REQUEST['location'] ) 			? sanitize_text_field( $_REQUEST['location'] ) 				: '';
$radius 			= isset( $_REQUEST['radius'] ) 				? sanitize_text_field( $_REQUEST['radius'] ) 				: $radius_max;
...
$cond 				= !empty( $_REQUEST['cond'] ) 				? sanitize_text_field( $_REQUEST['cond'] ) 					: '';
$image_only 		= !empty( $_REQUEST['image-only'] ) 		? true 														: false;
$sale_only 			= !empty( $_REQUEST['sale-only'] ) 			? true 														: false;
$urgent_only 		= !empty( $_REQUEST['urgent-only'] ) 		? true 														: false;
$negotiable_only	= !empty( $_REQUEST['negotiable-only'] ) 	? true 														: false;
$page 				= !empty( $_REQUEST['af_page'] ) 			? sanitize_text_field( $_REQUEST['af_page'] ) 				: 1;
$orderby 			= !empty( $_REQUEST['af_orderby'] ) 		? sanitize_text_field( $_REQUEST['af_orderby'] ) 			: '';
$layout 			= !empty( $_REQUEST['layout'] ) 			? sanitize_text_field( $_REQUEST['layout'] ) 				: adifier_get_option( 'default_search_listing' );
				...
				if( !empty( $values ) ){
					...
					if( in_array( $field->cf_type, array( 1, 2, 6, 7, 9, 10 ) ) ){
						...
						if( !empty( $values['min'] ) && !empty( $values['max'] ) ){
							$cf_filter['terms']	 = array( $values['min'], $values['max'] );
...
// 	}
// }
if( $map_source == 'google' && empty( $longitude ) && !empty( $location ) ){
...
if( !empty( $price ) ){
	...
	$price = map_deep( $price, 'sanitize_text_field' );
	$args['price'] = array(
...
if( !empty( $negotiable_only ) ){
	$args['meta_query'] = array( 
		array(
			'key' 	=> 'advert_negotiable',
			'value'	=> '1'
		) 
	);
}
