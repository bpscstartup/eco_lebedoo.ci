		...
		<div class="white-block-content">
			...
			<?php if( $use_google_location !== 'no' || $use_predefined_locations !== 'no' ): ?>
				<div class="form-group">
					...
					<?php else: ?>
						<?php adifier_taxonomy_filter( esc_html__( 'Location', 'adifier' ), 'location_id', 'advert-location', $location_id ); ?>
					<?php endif; ?>
				</div>
			<?php endif; ?>
				...
				<div class="form-group">
					...
					<?php if( adifier_get_option( 'price_filter_type' ) == 'slider' ): ?>
						<div class="slider-wrap slider-range">
							<div class="filter-slider price-filter-slider" data-range="true" data-min="0" data-max="<?php echo (int)$max_price ?>" data-refmax="<?php echo (int)$max_price ?>" <?php echo $price_slider_data ?>></div>
			...
			<div class="form-group">
				<?php if( adifier_is_allowed_ad_type(1) ): ?>
					<div class="styled-checkbox hidden show-sale-filter">
						<input type="checkbox" name="sale-only" value="1" id="sale-only" <?php echo  $image_only ? 'checked="checked"' : '' ?>>
						<label for="sale-only"><?php esc_html_e( 'Show ads on sale only', 'adifier' ) ?></label>
					</div>
				<?php endif; ?>
				...
				<div class="styled-checkbox">
					<input type="checkbox" name="negotiable-only" value="1" id="negotiable-only" <?php echo  $negotiable_only ? 'checked="checked"' : '' ?>>
					<label for="negotiable-only"><?php esc_html_e( 'Show negotiable ads only', 'adifier' ) ?></label>
				</div>
