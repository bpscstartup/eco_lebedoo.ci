 ...
 * @since 1.0.0
 */
class Adifier_Elementor_Button extends \Elementor\Widget_Base {
	...
	protected function render() {
		...
		if ( ! empty( $settings['link']['url'] ) ) {
            ...
            if( $settings['link']['url'] == '#submit' ){
                if( is_user_logged_in() ){
                    $settings['link']['url'] =  esc_url( add_query_arg( 'screen', 'new', get_author_posts_url( get_current_user_id() ) ));	
                }
                else{
                    $settings['link']['url'] = '#login';
                    $class .= ' af-modal';
                }
            }
            else if( is_user_logged_in() && in_array( $settings['link']['url'], array( '#login', '#register' ) )){
                $settings['link']['url'] = esc_url( get_author_posts_url( get_current_user_id() ));
            }
            else if( !is_user_logged_in() && in_array( $settings['link']['url'], array( '#login', '#register' ) )){
                $class .= ' af-modal';
            }     
			...
			$this->add_render_attribute( 'button', 'class', $class );
		}
