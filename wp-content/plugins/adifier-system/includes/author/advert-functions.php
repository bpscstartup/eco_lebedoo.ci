...
function adifier_save_advert(){
	...
	if( empty( $submit_terms ) || (!empty( $submit_terms ) && isset( $_POST['terms'] )) ){
		...
		if( !empty( $min_description_length ) && strlen( strip_tags( $description ) ) < $min_description_length ){
			$errors[] = sprintf( esc_html__( 'Minimum length of description must be %s chars', 'adifier' ), $min_description_length );
		}
...
/**
 * Check if there is an advert media
 */
if( !function_exists('adifier_has_image') ){
function adifier_has_image(){
    return has_post_thumbnail() || !empty( adifier_get_option( 'placeholder_thumbnail' )['id'] );
}
}
