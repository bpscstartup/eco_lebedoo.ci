...
function adifier_place_bid(){
	...
	if( is_user_logged_in() ){
		...
		if( !empty( $bid ) && adifier_validate_price_format( $bid, $currency ) ){
            ...
            $response['bids']  = adifier_bid_listing( $advert_id );
...
if(!function_exists('adifier_bid_listing')){
function adifier_bid_listing( $advert_id = '', $page = 1, $full = false, $ip = false, $per_page = 10, $is_admin = false ){
    global $wpdb;
...
function adifier_print_bidding_history( $advert_id = '', $page = 1, $full = false, $ip = false, $per_page = 10, $is_admin = false){
	global $wpdb;

	$response['message'] = adifier_bid_listing( $advert_id, $page, $full, $ip, $per_page, $is_admin );
...
if( !function_exists('adifier_update_auction') ){
function adifier_update_auction(){
	if( !isset( $_REQUEST['adifier_nonce'] ) || !wp_verify_nonce( $_REQUEST['adifier_nonce'], 'adifier_nonce' ) ){
		die();
	}
	if( !is_user_logged_in() ){
	...
	if( !empty( $advert_id ) ){
        echo json_encode([
            'bids'          => adifier_bid_listing( $advert_id ),
            'price'         => adifier_get_advert_price( $advert_id ),
            'min_bid'       => $min_bid,
            'min_bid_text'  => sprintf( esc_html__( 'Min bid: %s', 'adifier' ), strip_tags( adifier_price_format( $min_bid, $currency ) ) )
        ]);
	}
	
	die();
}
add_action( 'wp_ajax_adifier_update_auction', 'adifier_update_auction' );
}
...
function adifier_bid_history(){
	...
	if( !empty( $advert_id ) ){
		echo json_encode( adifier_print_bidding_history( $advert_id, $page, adifier_is_expired( $advert_id ) ? true : false ) );
	}
...
function adifier_bid_admin_action_remove(){
	...
	if( !$result ){
        $response['error'] = esc_html__( 'Could not delete the bid, try again', 'adifier' );
	}
    else{
