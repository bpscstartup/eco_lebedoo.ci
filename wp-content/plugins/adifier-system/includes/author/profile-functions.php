...
function adifier_update_account( $user_sent_id = '' ){
    ...
    $enable_tax_id = adifier_get_option( 'enable_tax_id' );

	if( ( ( empty( $email ) || !filter_var( $email, FILTER_VALIDATE_EMAIL ) ) || ( $enable_tax_id == 'yes' && !empty( $mandatory_fields['tax_id'] ) && empty( $tax_id ) ) || ( !empty( $mandatory_fields['first_name'] ) && empty( $first_name ) ) || ( !empty( $mandatory_fields['last_name'] ) && empty( $last_name ) ) || ( !empty( $mandatory_fields['phone'] ) && empty( $phone ) ) || ( $use_predefined_locations == 'yes' && empty( $location_id ) ) ||( $use_google_location == 'yes' && empty( $location['lat'] ) ) ) && empty( $user_sent_id ) ){
		$error = array();
		...
		if( $enable_tax_id == 'yes' && !empty( $mandatory_fields['tax_id'] ) && empty( $tax_id ) ){
			$error[] = esc_html__( 'Tax ID is required', 'adifier' );
		}
...
/**
 * Check if user is marked as unlimited for submitting adds
 */
if( !function_exists('adifier_is_user_unlimited') ){
function adifier_is_user_unlimited(){
    return get_user_meta( get_current_user_id(), 'af_unlimited', true ) == '1';
}
}
